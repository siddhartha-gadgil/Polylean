...
--proves fundamental relation between inverse and multiply
theorem inverse_mult {G : Graph V E} (x y z : V) (p : EdgePath G x y) (q : EdgePath G y z) (r : EdgePath G y x) (s : EdgePath G z y) (h₁ : r = inverse p) (h₂ : s = inverse q) :
      inverse (multiply p q) = multiply s r := by 
      induction p with
      | single x => 
            have p₁ : multiply (single x) q = q := by rfl
            rw[p₁]
            have p₂ : r = single x := by rw[h₁] ; rfl
            rw[p₂]
            rw[mult_const s]
            simp[h₂]
      | cons ex t₁ t₂ exy ih => 
          ...
          rw[p₃]
          --simp[inverse]
          have p₄ : r = multiply (inverse exy) (basicpath _ (G.bar ex)  t₂ (term_bar_equals_init t₁)) := by simp[h₁, inverse_lemma]
          rw[p₄]
          have p₅ : multiply s (inverse exy) = inverse ( multiply exy q) := by simp[ih q (inverse exy) s rfl h₂]
...
theorem inverse_involution  {G : Graph V E} {x y : V} (p : EdgePath G x y) : inverse (inverse p) = p := by 
  ...
  | cons ex h₁ h₂ exy ih => 
    ...
    rw[p₁]
    let k := inverse_mult _ _ _ (inverse exy)  (inverse (basicpath _ ex h₁ h₂)) exy (inverse (inverse (basicpath _ ex _ h₂))) (Eq.symm ih) rfl
    rw[k]
    have p₂ : inverse (inverse (basicpath _ ex h₁ h₂)) = basicpath _ ex h₁ h₂ := by 
      ...
      rfl
    rw[p₂]
    apply basicpath_mult ex h₁ h₂ exy
...
theorem homotopy_right_mult {G : Graph V E} {x y z : V} (p₁ p₂ : EdgePath G x y) (q : EdgePath G y z) (h :homotopy p₁ p₂):
         ...
         let s₁ := inverse_mult x y z p₁ q (inverse p₁) (inverse q) rfl rfl 
         let s₂ := inverse_involution (multiply p₁ q)
         let s₃ := Eq.trans (Eq.symm s₂) (congrArg inverse s₁)
         let t₁ := inverse_mult x y z p₂ q (inverse p₂) (inverse q) rfl rfl 
