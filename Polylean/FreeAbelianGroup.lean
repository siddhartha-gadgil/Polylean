...
theorem zsmul_succ (n: ℤ) (x : A) : zsmul (n+1) x = x + zsmul n x  := by
    ...
    case negSucc k =>
      induction k with
      | zero =>
        show zsmul (Int.ofNat _) _ = _
        rw [abg.zsmul_neg', abg.zsmul_succ']
        simp
      | succ l _ =>
        show zsmul (Int.negSucc l) _ = _
        rw [abg.zsmul_neg', abg.zsmul_neg', abg.zsmul_succ' l.succ]
